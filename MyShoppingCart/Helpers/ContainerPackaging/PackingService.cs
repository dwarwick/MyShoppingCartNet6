...
namespace MyshoppingCart.Helpers.ContainerPackaging
{
	/// <summary>
	/// The container packing service. https://github.com/davidmchapman/3DContainerPacking
	/// </summary>
	public static class PackingService
	{
		...
		public static List<ContainerPackingResult> Pack(List<Container> containers, List<Item> itemsToPack, List<int> algorithmTypeIDs)
		{
			...
			Parallel.ForEach(containers, container =>
			{
				...
				containerPackingResult.Name = container.Name;
				containerPackingResult.Price = container.Price;
				...
				Parallel.ForEach(algorithmTypeIDs, algorithmTypeID =>
				{
					...
					{
						items.Add(new Item(item.ID, item.Dim1, item.Dim2, item.Dim3, item.Weight, item.Quantity));
					});
